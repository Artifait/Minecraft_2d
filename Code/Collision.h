...
void mergeOverlappingRects(vector<rect>& rects, int SizeBlock) {//оно обьединяет блоки в big блоки для fast проверки на коллизии
	//vector<v2f> good_rect_id;
	map<v2f, rect> mapa_rects;
	for (auto& rect_now : rects)
	{
		mapa_rects[v2f(rect_now.body_rect.getPosition()) / SizeBlock] = rect_now;
	}
	rects.clear();
	...
	while (mapa_rects.size() > GOOD_ITERATION)
	{
		...
		while (GOOD_SEARHING) {
			if (mapa_rects.find(id_rect + offset_searching) != mapa_rects.end())
			{
				...
				r_now->setsize(offset_searching * SizeBlock + r_now->body_rect.getSize());
