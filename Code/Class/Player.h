...
public:
	...
	void update_state(v2f world_pos_mouse, RenderWindow& window)
	{
		...
		if (Mouse::isButtonPressed(Mouse::Left) && !mouse_clicked)
		{
			...
			line_poisk[1].position = Give_center_player() + (world_pos_mouse - Give_center_player()).getNormalized() * Block::Size_block * 2.4f;
			line_poisk[0].color = Color::Red;
			line_poisk[1].color = Color::Red;
			//window.draw(line_poisk);

			Block* bl_found = poisk_block(line_poisk, *player_world);
			if (bl_found != nullptr)
			{
